...
const results = vm.runInContext(`(() => {
  const sopranos = TITLES.find(t => t.title === "The Sopranos");
  const succession = TITLES.find(t => t.title === "Succession");
  const severance = TITLES.find(t => t.title === "Severance");
  return {
    search: TITLES.filter(t => t.title.toLowerCase().includes("sopranos")).length,
